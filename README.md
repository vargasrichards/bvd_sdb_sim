...
* `evd_sdb_sim_parameters.xlsx` contains a list of model parameters which the user can modify as wished. Note that the `n_sim` parameter heavily affects computing power requirements: on a cheap 2020 Laptop, 1000 simulation runs are OK for about 20-30 individual scenarios (and take <10 min to run), but anything beyond this may require a powerful PC or a high-performance cluster.
